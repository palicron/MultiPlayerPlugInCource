...
ABlasterCharacter::ABlasterCharacter()
{
	...
	*/
	...
	for (auto boxes : HitCollisionBoxes)
	{
		if(boxes.Value)
		{
			boxes.Value->SetCollisionObjectType(ECC_HitBox);
			boxes.Value->SetCollisionResponseToAllChannels(ECR_Ignore);
			boxes.Value->SetCollisionResponseToChannel(ECC_HitBox, ECR_Block);
			boxes.Value->SetCollisionEnabled(ECollisionEnabled::NoCollision);
		}
	}
}
...
void ABlasterCharacter::PlaySawpMontage()
{
	UAnimInstance* AnimInstance= GetMesh()->GetAnimInstance();
	if(AnimInstance && SawpMontage)
	{
		AnimInstance->Montage_Play(SawpMontage);
	}
}
...
void ABlasterCharacter::Elim(bool bPlayerLeftGame)
{
	...
	MulticasElim(bPlayerLeftGame);
	
}

void ABlasterCharacter::MulticasElim_Implementation(bool bPlayerLeftGame)
{
...
void ABlasterCharacter::EquipButtonPressed()
{
	...
	if(Combat)
	{
		...
		const bool bSawp = Combat->ShouldSwapWeapons() && !HasAuthority() && Combat->CombatState == ECombatState::ECS_Unoccupied
		&& OverLappingWeapon == nullptr;
		
		if(bSawp)
		{
			PlaySawpMontage();
			Combat->CombatState = ECombatState::ECS_SwappingWeapones;
